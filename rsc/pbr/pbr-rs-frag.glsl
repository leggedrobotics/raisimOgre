...
void main()
{
    ...
    vec4 mSample = texture2D(u_MetallicSampler, v_UV);
    vec4 rSample = texture2D(u_RoughnessSampler, v_UV);
    perceptualRoughness = rSample.r * perceptualRoughness;
    metallic = mSample.r * metallic;
#endif
...
#ifdef USE_IBL
    color += 0.6 * getIBLContribution(pbrInputs, n, reflection);
#endif
